...
In a previous post I showed a generic approach to hashing (see https://blog.infotraining.pl/how-to-hash-objects-without-repetition). That implementation required a `tied()` member that exposes an object's members as an `std::tuple` of references. With such a `tied()` implementation, we could provide a specialization of `std::hash<T>` to compute an object's hash.
...
Information obtained through reflection can be used for performing compile-time checks, generating code, or implementing custom behavior based on the structure of the code. The reflection value can be used to produce gramatical elements using so-called **splicers** with `[: reflection-value :]` syntax, this allows to generate code based on the structure of the code at compile time.
...
Let us see a simple example that allow us to get member values using indexes:
...
At this point, we can calculate the combined hash value for all non-static data members. However, we're not finished yet. We haven't considered the base classes or structs that should also be included in the hash calculation. With reflection, this task is not as challenging as it might seem.
...
We now have an almost complete solution. The final consideration is how to enable specialization of `std::hash` for a custom type. One approach is to opt-in to our hashing implementation by defining an alias, `enabled_for_hashing`, within a custom class. This alias activates the generation of a hash value using our implementation.
...
As a next step we can extend our implementation with feature like attributes that will allow to discard some values for calculating hash, but I think it is a nice subject for a next post.
